...
export class Cart {
  ...
  totalPrice: number = 0;
    ...
    //  The constructor receives a Repository object through dependency injection, which is used to get any
    //  stored cart data for the user’s session.If there is data available, it is used to populate the cart, and the update
    //  method is called in order to update the properties that are used in data bindings and, in doing so, trigger
    //  Angular to reevaluate those bindings to present the modified cart to the user.
